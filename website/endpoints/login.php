<?php
    ...
    {   
        ...
        $check_credentials_query="SELECT `user_id`  FROM user WHERE email='$username' AND password = '$password'";
        $check_cred_res = mysqli_query($conn, $check_credentials_query);
        if($check_cred_res){
            if(mysqli_num_rows($check_cred_res) > 0){
    
                echo json_encode(array("status" => "SUCCESSFUL", "message" => "Successful login", "status_code" => 200,"user" => $username));
